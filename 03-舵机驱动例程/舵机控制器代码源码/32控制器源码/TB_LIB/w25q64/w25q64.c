...
//2010/6/13
//V1.0
u16 W25Q_TYPE=W25Q64;//默认就是25Q64
//4Kbytes为一个Sector
//16个扇区为1个Block
//W25X16
...
void W25Q_Init(void) {
	...
	W25Q_TYPE=W25Q_ReadID();//读取FLASH ID.
}  
...
{ 
	...
	while(1) 
	{	
		W25Q_Read(W25Q_BUF,secpos*4096,4096);//读出整个扇区的内容
		...
		if(i<secremain)//需要擦除
		{
			...
			W25Q_Write_NoCheck(W25Q_BUF,secpos*4096,4096);//写入整个扇区  

		}else W25Q_Write_NoCheck(pBuffer,WriteAddr,secremain);//写已经擦除了的,直接写入扇区剩余区间. 				   
...
{   
	while ((W25Q_ReadSR()&0x01)==0x01);   // 等待BUSY位清空
}  
//进入掉电模式
