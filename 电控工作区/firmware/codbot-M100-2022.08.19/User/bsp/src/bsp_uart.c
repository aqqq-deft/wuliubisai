...
UartFrame_t  g_uartFrame;       /*用于串口树莓派数据命令接受*/
UartState_t  g_uartState;       /*串口接受数据状态*/
...
static void UartVarInit(void)
{
	...
	g_tUart1.ReciveNew = uart_ReciveNew;		/* 接收到新数据后的回调函数 */
...
/* 使能 GPIO TX/RX 时钟 */
	...
	// USART_CR1_PEIE | USART_CR1_RXNEIE
	SET_BIT(USART2->CR1, USART_CR1_PEIE | USART_CR1_RXNEIE);	/* 使能PE. RX接受中断 */
#endif
